...
from numba import cuda, void, float32, float64, vectorize
import math
...
def _psf(meshinfo, angles, diffraction_orders, focal_plane=False):
    ...
    blockspergrid = (Nx // 16, Ny // 16)
    _numba_psf[blockspergrid, threadsperblock](psf, x, y, spacing_x, spacing_y, diffraction_orders[0],
                                               diffraction_orders[-1] + 1, Nx, Ny, width_x, width_y, mesh_ratio)

    # Contribution from core
    psf_core = cupy.exp(-width_x * (x - 0.5 * Nx - 0.5) ** 2
...
@cuda.jit
def _numba_psf(in_x, in_y, psf, spacing_x, spacing_y, O_begin, O_end, Nx, Ny, width_x, width_y, mesh_ratio):
    ...
    for order in range(cupy.int32(O_begin), cupy.int32(O_end), 1):
        ...
        for i in range(cupy.int32(len(spacing_x))):
            ...
            temp += math.exp(-width_x * x_centered * x_centered - width_y * y_centered * y_centered) * intensity
